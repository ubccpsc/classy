...
export class GithubAutoTest extends AutoTest implements IGithubTestManager {
    ...
    public async handleCommentEvent(info: ICommentEvent): Promise<boolean> {
        ...
        try {
            ...
            Log.info("GithubAutoTest::handleCommentEvent(..) - start; course: " + this.courseId + "; commit: " + info.commitSHA + "; user: " + info.userName);
    ...
    private async savePushInfo(info: IContainerInput) {
        try {
            Log.trace("AuGithubAutoTesttoTest::savePushInfo(..) - commit: " + info.pushInfo.commitSHA);
