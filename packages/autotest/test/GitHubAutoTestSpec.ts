...
describe("GitHubAutoTest", () => {
	...
	function stubDependencies() {
		...
		at["postToGitHub"] = function (info: CommitTarget, message: IGitHubMessage): Promise<boolean> {
			...
			Log.test("stubbed postToGitHub(..) - done; length: " + gitHubMessages.length + "; content: " + JSON.stringify(gitHubMessages));
	...
	it("Should be able to receive a valid comment event from a student for an open deliverable.", async () => {
		...
		const ce: CommitTarget = {
			...
			delivId: "d0",
			postbackURL:
				"https://github.students.cs.ubc.ca/api/v3/repos/classytest/PostTestDoNotDelete/commit/c35a0e5968338a9757813b58368f36ddd64b063e/comments",
			timestamp: TS_IN,
			cloneURL: "https://cloneURL",
		};
