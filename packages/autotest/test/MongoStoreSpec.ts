...
describe("MongoStore", () => {
	...
	it("Should be able to retrieve a comment event.", async () => {
		...
		// TEST
		const actual = await ds.getCommentRecord(TestData.commentRecordStaffA.commitURL, TestData.commentRecordStaffA.delivId, "standard");
	...
	it("Should return null for a comment event that has not been saved.", async () => {
		...
		// TEST
		const actual = await ds.getCommentRecord(TestData.commentRecordUserA.commitURL, TestData.commentRecordUserA.delivId, "standard");
