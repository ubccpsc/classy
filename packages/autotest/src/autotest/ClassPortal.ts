...
export class ClassPortal implements IClassPortal {
	...
	public async sendGrade(grade: AutoTestGradeTransport): Promise<Payload> {
		// really just a mechanism to report more verbose error
	...
	public async formatFeedback(res: AutoTestResultTransport): Promise<string | null> {
		...
		Log.trace("ClassPortal::formatFeedback(..) - start; deliv: " + res.delivId + "; URL: " + res.commitURL);
		...
		Log.trace(
			"ClassPortal::formatFeedback(..) - repo: " +
				res.repoId +
				"; SHA: " +
				Util.shaHuman(res.commitSHA) +
				"; feedback: " +
				msg +
				"; took: " +
				Util.took(start)
		...
		Log.info(
			"ClassPortal::formatFeedback(..) - deliv: " +
				res.delivId +
				"; repo: " +
				res.repoId +
				"; feedback: " +
	...
	public async sendResult(result: AutoTestResult): Promise<Payload> {
		// really just a mechanism to report more verbose errors
		const start = Date.now();
		...
		try {
			...
			Log.trace(
				"ClassPortal::sendResult(..) - sending to: " +
					url +
					" for deliv: " +
					result.delivId +
					"; repoId: " +
			...
			if (typeof json.success !== "undefined") {
				Log.info(
					"ClassPortal::sendResult(..) - result accepted; SHA: " +
						Util.shaHuman(result.input.target.commitSHA) +
						"; took: " +
						Util.took(start)
				);
				return json;
	...
	public async getResult(delivId: string, repoId: string, sha: string, ref?: string): Promise<AutoTestResultTransport | null> {
		if (!ref || ref === "") {
			// undefined, null, "" should match any result
			ref = "<ANY>";
		}
	...
	public async requestFeedbackDelay(
		...
		try {
			const opts: RequestInit = {
				...
				body: JSON.stringify({ delivId: delivId, personId: personId, timestamp: timestamp }),
			};
			...
			Log.trace(
				"ClassPortal::requestFeedbackDelay(..) - types; json: " +
					typeof json +
					"; json.accepted: " +
					typeof json?.accepted +
					"; json.message: " +
					typeof json?.message +
					"; json.fullMessage: " +
			...
			if (typeof json?.accepted === "boolean" && (typeof json?.message === "string" || typeof json?.fullMessage === "string")) {
				resp = {
