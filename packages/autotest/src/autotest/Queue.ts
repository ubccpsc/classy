...
export class Queue {
	...
	public remove(info: ContainerInput): ContainerInput | null {
		for (let i = this.data.length - 1; i >= 0; i--) {
			...
			if (
				queued.target.commitURL === info.target.commitURL &&
			...
			if (info.target.timestamp < oldestJob.target.timestamp) {
				...
				Log.info(
					"Queue::replaceOldestForPerson( " +
						...
						this.name +
						"; replacing sha: " +
						Util.shaHuman(oldestJob.target.commitSHA) +
						"; with sha: " +
	...
	public numberJobsForPerson(input: ContainerInput): number {
		...
		if (count > 1 || input.target?.adminRequest === true) {
			Log.info(
				"Queue::numberJobsForPerson( " +
					input.target?.personId +
					" ) - queue: " +
					this.name +
					"; isAdmin: " +
					input.target?.adminRequest +
					"; count: " +
					count
			);
		} else {
	...
	public load() {
		try {
			...
			for (const data of store.data) {
				Log.info("Queue::load() - queue: " + this.name + "; add queued to TAIL: " + Util.shaHuman(data.target.commitSHA));
