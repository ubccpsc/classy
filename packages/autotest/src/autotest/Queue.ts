...
export class Queue {
    ...
    public load() {
        try {
            ...
            Log.info("AutoTest::load() - rehydrated store: " + store);
            Log.info("AutoTest::load() - for testing only; not adding rehydrated elements to queue yet");
            ...
            Log.info("AutoTest::load() - ERROR rehydrating queue: " + err.message);
        }
    }
}
