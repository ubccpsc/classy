...
export abstract class AutoTest implements IAutoTest {
	...
	public addToExpressQueue(input: ContainerInput): void {
		Log.info(
			"AutoTest::addToExpressQueue(..) - start" +
				"; deliv: " +
				input.target.delivId +
				"; repo: " +
				input.target.repoId +
				"; SHA: " +
	...
	public addToStandardQueue(input: ContainerInput, forceAdd: boolean): void {
		Log.info(
			"AutoTest::addToStandardQueue(..) - start" +
				"; deliv: " +
				input.target.delivId +
				"; repo: " +
				input.target.repoId +
		...
		try {
			...
			if (forceAdd === true) {
				Log.info(
					"AutoTest::addToStandardQueue(..) - repo: " +
						input.target.repoId +
						"; has # " +
						stdJobCount +
						" std jobs queued and # " +
	...
	public addToLowQueue(input: ContainerInput): void {
		...
		try {
			...
			if (this.expressQueue.indexOf(input) >= 0 || this.standardQueue.indexOf(input) >= 0) {
				...
				Log.info(
					"AutoTest::addToLowQueue(..) - skipped; " +
						"job already on standard or express queue; SHA: " +
						Util.shaHuman(input.target.commitSHA)
				);
				return;
			}
			...
			if (lowJobCount >= this.MAX_LOW_JOBS) {
				Log.warn(
					"AutoTest::addToLowQueue(..) - user has _many_ ( " +
						lowJobCount +
						") queued jobs, " +
						"will replace oldest job with this job; repo: " +
						input.target.repoId +
						"; person: " +
				...
				if (queue.length() > 0 && that.hasCapacity() === true) {
					...
					Log.info(
						"AutoTest::tick::tickQueue(..)         [JOB] - job start: " +
							queue.getName() +
							"; deliv: " +
							(info.target.delivId + ";").padEnd(8, " ") +
							" repo: " +
							(info.target.repoId + ";").padEnd(18, " ") +
							" SHA: " +
							Util.shaHuman(info.target.commitSHA) +
							"; # running: " +
							totalJobsRunning +
							"; # queued: " +
	...
	protected isCommitExecuting(input: ContainerInput): boolean {
		try {
			for (const execution of this.jobs) {
				if (
					execution.target.commitURL === input.target.commitURL &&
	...
	private async handleExecutionComplete(data: AutoTestResult): Promise<void> {
		try {
			...
			if (
				typeof data.commitSHA === "undefined" ||
			...
			Log.info(
				"AutoTest::handleExecutionComplete(..) - start" +
					": deliv: " +
					data.delivId +
					"; repo: " +
					data.repoId +
			...
			Log.info(
				"AutoTest::handleExecutionComplete(..) [JOB] - job complete;   deliv: " +
					(data.delivId + ";").padEnd(8, " ") +
					" repo: " +
					(data.repoId + ";").padEnd(18, " ") +
					" SHA: " +
					Util.shaHuman(data.commitSHA) +
					"; wait: " +
					Util.tookHuman(data.input.target.timestamp, data.input.target.tsJobStart, true, true) +
					"; exec: " +
					Util.tookHuman(data.input.target.tsJobStart, data.output.timestamp, true, true)
			);
		} catch (err) {
	...
	private async handleTick(job: GradingJob) {
		...
		try {
			...
			if (containerConfig !== null && input.containerConfig.dockerImage !== containerConfig.dockerImage) {
				Log.info(
					"AutoTest::handleTick(..) - stale job; old container: " +
						input.containerConfig.dockerImage +
						"; new container: " +
						containerConfig.dockerImage
				);
		...
		Log.info(
			"AutoTest::handleTick(..) - start; deliv: " +
				input.target.delivId +
				"; repo: " +
				input.target.repoId +
				"; SHA: " +
		...
		try {
			...
			Log.info(
				"AutoTest::handleTick(..) - prepared; deliv: " +
					input.target.delivId +
					"; repo: " +
					input.target.repoId +
					"; SHA: " +
			...
			Log.info(
				"AutoTest::handleTick(..) - executed; deliv: " +
					input.target.delivId +
					"; repo: " +
					input.target.repoId +
					"; SHA: " +
		...
		Log.info(
			"AutoTest::handleTick(..) - done; deliv: " +
				input.target.delivId +
				"; repo: " +
				input.target.repoId +
				"; SHA: " +
	...
	private clearExecution(commitURL: string, delivId: string, ref: string | undefined): boolean {
		...
		for (let i = this.jobs.length - 1; i >= 0; i--) {
			...
			if (execution !== null) {
				if (execution.target.commitURL === commitURL && execution.target.delivId === delivId && execution.target.ref === ref) {
					// remove this one
