...
export class GradingJob {
	...
	public async prepare(): Promise<void> {
		try {
			...
			await Promise.all([fs.mkdirp(this.path + "/staff"), fs.mkdirp(this.path + "/student"), fs.mkdirp(this.path + "/admin")]);
	...
	public async run(docker: Docker): Promise<AutoTestResult> {
		...
		const container = await docker.createContainer({
			...
			Env: [`ASSIGNMENT=${this.input.target.delivId}`, `EXEC_ID=${this.id}`, `INPUT=${JSON.stringify(this.input)}`],
			HostConfig: {
				AutoRemove: true,
				Binds: [`${hostDir}/assn:/assn`, `${hostDir}:/output`],
				ExtraHosts: [Config.getInstance().getProp(ConfigKey.hostsAllow)],
		...
		if (exitCode === -10) {
			...
			Log.warn(
				"GradingJob::run() - No grading report for repo: " +
		...
		if (out.state !== ContainerState.SUCCESS || (reportRead === true && out.report.result !== ContainerState.SUCCESS)) {
			const msg =
				"GradingJob::run() - after; repo: " +
				this.input.target.repoId +
				"; delivId: " +
				this.input.target.delivId +
				"; sha: " +
