...
export class GitHubUtil {
	...
	public static parseDeliverableFromComment(message: any, delivIds: string[]): string | null {
		...
		Log.trace(
			"GitHubUtil::parseDeliverableFromComment(..) - ids: " +
				JSON.stringify(delivIds) +
				"; matches: " +
		...
		if (parsedDelivId === null) {
			Log.trace("GitHubUtil::parseDeliverableFromComment() - NO MATCH; input: " + msg + "; options: " + JSON.stringify(delivIds));
		} else {
	...
	public static async processComment(payload: any): Promise<CommitTarget> {
		try {
			...
			let orgId;
			try {
				orgId = payload.repository.full_name.substr(0, payload.repository.full_name.lastIndexOf(payload.repository.name) - 1);
				Log.trace(
					"GitHubUtil::processComment(..) - full_name: " +
				...
				Log.info(
					"GitHubUtil::processComment(..) - start; repo: " +
						payload.repository.name +
						"; person: " +
						requester +
						"; SHA: " +
			...
			Log.info(
				"GitHubUtil.processComment(..) - done" +
					"; repo: " +
					payload.repository.name +
					"; person: " +
					requester +
	...
	public static async processPush(payload: any, portal: IClassPortal): Promise<CommitTarget | null> {
		try {
			...
			let org;
			try {
				org = payload.repository.full_name.substr(0, payload.repository.full_name.lastIndexOf(repo) - 1);
				Log.trace("GitHubUtil::processPush(..) - full_name: " + payload.repository.full_name + "; org: " + org + "; name: " + repo);
			} catch (err) {
			...
			if (payload.deleted === true && payload.head_commit === null) {
				...
				Log.info("GitHubUtil::processPush(..) - skipped; repo: " + repo + "; branch removed: " + projectURL);
				return null;
			}
			...
			if (typeof payload.commits !== "undefined" && payload.commits.length > 0) {
				...
				shas = shas.trim();
				Log.trace(
					"GitHubUtil::processPush(..) - regular push; repo: " +
						repo +
						"; # commits: " +
			...
			Log.info(
				"GitHubUtil::processPush(..) - done; person: " +
					pusher?.personId +
					"; repo: " +
					repo +
					"; SHA: " +
