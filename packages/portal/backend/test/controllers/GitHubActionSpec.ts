...
describe("GitHubActions", () => {
	...
	it("Should not be possible to find a team that does not exist.", async function () {
		const team = await gh.getTeamByName(TestHarness.INVALIDTEAMNAME);
	...
	it("Should be possible to find a repo that exists.", async function () {
	...
	it("Should be possible to add members to a team that exists.", async function () {
		const addMembers = await gh.addMembersToTeam(TEAMNAME, [TestHarness.GITHUB1.github, TestHarness.GITHUB2.github]);
	...
	it("Should be possible to add a team to a repo.", async function () {
		...
		expect(teamAdd.githubTeamNumber).to.equal(team.githubTeamNumber);
	...
	it("Should be able to create a team, add users to it, and add it to the repo.", async function () {
		...
		const addMembers = await gh.addMembersToTeam(val.teamName, [TestHarness.GITHUB1.github, TestHarness.GITHUB2.github]);
		...
		const staffTeam = await gh.getTeamByName("staff");
		...
		expect(staffAdd.githubTeamNumber).to.equal(staffTeam.githubTeamNumber);
		Log.test("Team staff added to repo");
	}).timeout(TIMEOUT);

	it("Should be possible to get a team number for a team that exists.", async function () {
		const team = await gh.getTeamByName(TestHarness.TEAMNAME1);
		expect(team.teamName).to.equal(TestHarness.TEAMNAME1);
	}).timeout(TIMEOUT);
	...
	it("Should be able to add a team to a repo.", async function () {
		...
		Log.test("Getting team name");
		const team = await gh.getTeamByName(TEAMNAME);
		Log.test("Team name: " + TEAMNAME);
		expect(team.teamName).to.be.an("string");
		expect(team.teamName).to.be.equal(TEAMNAME);
	...
	it("Should be able to clone a source repository, and select files to create a new repository.", async function () {
		...
		const newName =
			Config.getInstance().getProp(ConfigKey.githubHost) + "/" + Config.getInstance().getProp(ConfigKey.org) + "/" + REPONAME3;
		...
		const importTests: Array<[string, string]> = [
			...
			[
				TestHarness.REPONAMEREAL_TESTINGSAMPLE +
					".git#" +
					TestHarness.REPOBRANCHREAL_TESTINGSAMPLE +
					":" +
					TestHarness.REPOSUBDIRREAL_TESTINGSAMPLE,
				undefined,
			],
	...
	it("Should be able to create a repo, a team, link, and set permissions", async function () {
		...
		const addMembers = await gh.addMembersToTeam(githubTeam.teamName, [TestHarness.GITHUB1.github, TestHarness.GITHUB2.github]);
