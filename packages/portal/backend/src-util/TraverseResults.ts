		...
		for (const url of URLs) {
			...
			for (const result of resultsForRepo) {
				...
				if (result.output.report.scoreOverall >= highestGradeBeforeDeadline.output.report.scoreOverall) {
					Log.info(
						"\tHigher grade found: " +
							highestGradeBeforeDeadline.output.report.scoreOverall +
							"; with: " +
							result.output.report.scoreOverall +
							"; prev sha: " +
							highestGradeBeforeDeadline.commitSHA +
							"; new sha: " +
							result.commitSHA
					);
					highestGradeBeforeDeadline = result;
			...
			if (highestGradeBeforeDeadline.commitURL === url) {
				...
				Log.warn(
					"Grade URL DOES NOT MATCH highest result URL for deliv: " +
						deliv.id +
						"; repo: " +
						repoId +
						"; grade url: " +
						url +
						"; highest result url: " +
						highestGradeBeforeDeadline.commitURL
				);
