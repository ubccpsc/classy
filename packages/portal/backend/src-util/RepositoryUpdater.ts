...
export class RepositoryUpdater {
    ...
    private async deleteUnwantedBranches(repo: Repository, gha: IGitHubActions): Promise<boolean> {
         ...
         */
        const BRANCH_NAMES_TO_DELETE = ["feature/pull_request_template_improvement", "removeFolderTest", "remove-dynamic-tests", "master", "errortype", "23W2EmptyChanges", "empty-repo"];
        ...
        Log.info("RepositoryUpdater::deleteUnwantedBranches() - repo: " + repo.id +
            "; branchesToRemove: " + JSON.stringify(branchesToRemove) + "; branchesToKeep: " + JSON.stringify(branchesToKeep));

        let allSuccess = true;
        for (const branch of branchesToRemove) {
            if (this.DRY_RUN === false) {
                Log.info("RepositoryUpdater::deleteUnwantedBranches() - DRY_RUN === false; removing branch; repo: " +
                    repo.id + "; branch: " + branch);
                try {
                ...
                Log.info("RepositoryUpdater::deleteUnwantedBranches() - DRY_RUN === true; should have deleted branch - repo: " +
                    repo.id + "; branch: " + branch);
            }
        }
