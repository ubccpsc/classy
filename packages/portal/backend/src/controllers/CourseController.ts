...
export class CourseController implements ICourseController {
    ...
    public async computeNames(deliv: Deliverable, people: Person[]): Promise<{teamName: string | null, repoName: string | null}> {
        ...
        // TODO this poses a potential race condition.
        // There are two easy ways to fix this
        //  1. Add a team counter to the database and leverage
        //       mongodb's `db.collection.findAndModify()`
        //  2. Use the unique 4 digit team number generated by GitHub
        //       as a team is inevitably made _before_ team creation.
        //       This necessitates repository name not being calculated here.
        let postfix: string = people.length === 1 ? 'id' : 'team';
        postfix += `${(await this.tc.getAllTeams()).length}`;
        // let postfix = '';
