...
export class DatabaseController {
					...
					pipeline: [
						{
							$match: {
								$expr: {
									$and: [{ $eq: ["$repoId", "$$srcRepo"] }, { $eq: ["$output.report.scoreOverall", "$$score"] }],
								},
							},
						},
					],
	...
	public async readRecords(column: string, kind: QueryKind, limitResults: boolean, query: {}, sort?: {}): Promise<any[]> {
		try {
			...
			if (records === null || records.length === 0) {
				Log.trace(
					"DatabaseController::readRecords(..) - done; no records found for: " +
				...
				Log.trace(
					"DatabaseController::readRecords(..) - done; query: " +
						JSON.stringify(query) +
						"; # records: " +
						records.length +
						". took: " +
	...
	public async getResults(delivId: string, repoId: string): Promise<Result[]> {
		...
		const latestFirst = { "input.target.timestamp": -1 }; // most recent first
		const results = (await this.readRecords(
			this.RESULTCOLL,
			QueryKind.SLOW,
			false,
		...
		Log.trace(
			"DatabaseController::getResults( " + delivId + ", " + repoId + " ) - done; #: " + results.length + "; took: " + Util.took(start)
		);
	...
	public async getResultsForRepo(repoId: string): Promise<Result[]> {
		...
		Log.trace("DatabaseController::getResultsForRepo( " + repoId + " ) - done; #: " + results.length + "; took: " + Util.took(start));
	...
	public async getResultsForDeliverable(delivId: string): Promise<Result[]> {
		...
		Log.trace(
			"DatabaseController::getResultsForDeliverable( " + delivId + " ) - done; #: " + records.length + "; took: " + Util.took(start)
		);
