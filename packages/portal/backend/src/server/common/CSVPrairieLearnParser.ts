	...
	public async processGrades(requesterId: string, path: string): Promise<boolean[]> {
		try {
			...
			for (const row of data) {
				if (typeof row.UID === "undefined" && typeof row.UIN === "undefined" && typeof row.Role === "undefined") {
			...
			for (const deliv of allDelivs) {
				const row = data[0];
				if (deliv.id !== "UID" && deliv.id !== "UIN" && deliv.id !== "Role" && typeof row[deliv.id] !== "undefined") {
