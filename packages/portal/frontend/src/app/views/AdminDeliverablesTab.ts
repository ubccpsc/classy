...
export class AdminDeliverablesTab extends AdminPage {
	...
	private render(deliverables: DeliverableTransport[]) {
		...
		for (const deliv of deliverables) {
			...
			const sub =
				"Opens: " + new Date(deliv.openTimestamp).toLocaleString() + "; Closes: " + new Date(deliv.closeTimestamp).toLocaleString();
	...
	public renderEditDeliverablePage(deliv: DeliverableTransport) {
		...
		const state = {
			checkedItemSha: selectedDockerImage,
		};
		...
		(document.querySelector("#btnNewImage") as OnsButtonElement).onclick = function () {
			UI.pushPage("createDockerImage.html")
				.then(function () {
					let imageSha: string;
		...
		try {
			...
			if (lines.length > SUCCESS_LINE && lines[lines.length - SUCCESS_LINE].startsWith("Successfully built")) {
				...
				return Promise.reject(
					new Error(
						"Failed to read image SHA from build log. " +
