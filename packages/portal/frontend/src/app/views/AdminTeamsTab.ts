...
export class AdminTeamsTab extends AdminPage {
	...
	public async init(opts: any): Promise<void> {
		...
		if (typeof opts.delivId === "undefined") {
			const defaultDelivProvisions = provisionDelivs.some((deliv) => deliv.id === this.course.defaultDeliverableId);

			const projectProvisions = provisionDelivs.some((deliv) => deliv.id === "project");
		...
		const updateTeamTable = function () {
			...
			Log.info("AdminTeamsTab::init(..)::updateTeamTable() - deliv: " + delivValue + "; status: " + statusValue);
	...
	private renderTeams(teams: TeamTransport[], delivId: string, labSection: string): void {
		...
		for (const team of teams) {
			...
			if (delivId === team.delivId && team.people.length > 0) {
				...
				if (labSection === labs || labSection === "-All-" || (labSection === "-Unspecified-" && (labs === "" || labs === null))) {
					st.addRow(row);
				}
				// st.addRow(row);
	...
	private getLabsCell(people: string[]): string {
		const labs = people.map((personId) => this.getPerson(personId)?.labId).filter((lab) => !!lab);
	...
	private renderIndividuals(teams: TeamTransport[], students: StudentTransport[], delivId: string, labSection: string): void {
		...
		for (const student of students) {
			if (studentsOnTeams.indexOf(student.id) < 0) {
				...
				if (delivId !== "-None-") {
					if (labSection === lab || labSection === "-All-" || (labSection === "-Unspecified-" && (lab === "" || lab === null))) {
						st.addRow(row);
					}
					// st.addRow(row);
					listContainsStudents = true;
		...
		try {
			...
			if (response.status === 200) {
				...
				if (typeof json.success !== "undefined" && Array.isArray(json.success)) {
					Log.trace("AdminTeamsTab::getTeams(..)  - worked; # teams: " + json.success.length + "; took: " + UI.took(start));
					return json.success;
				} else {
