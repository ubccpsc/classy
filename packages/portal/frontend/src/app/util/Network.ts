...
export class Network {
	...
	//     console.log("Network::detectUnauthenticated( " + url + "isAuthenticated ) - start");
	//     const USE_REAL = true;
	//     if (USE_REAL === true) {
	...
	//             // onError(err.message);
	//         });
	//         console.log("Network::detectUnauthenticated( " + url + "isAuthenticated ) - end")
	//     }
	// }
	...
	//             console.log("Network::handleRemote() WARNING: Response status: " + data.status);
	//             throw new Error("Network::handleRemote() - API ERROR: " + data.status);
	//         } else if (data.status !== 200 && data.status === 405 || data.status === 401) {
	//             console.error("Network::getRemotePost() Permission denied for your role.");
