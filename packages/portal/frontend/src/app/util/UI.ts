...
export class UI {
	...
	public static createListHeader(text: string): HTMLElement {
		const taskHeader = ons.createElement("<ons-list-header>" + text + "</ons-list-header>") as HTMLElement;
		return taskHeader;
	}
	...
	public static templateConfirm(template: string, options: { header?: string; listContent?: Array<{ text: string; subtext?: string }> }) {
		return ons
			.createElement(template, { append: true })
			.then(function (classlistDialog: any) {
