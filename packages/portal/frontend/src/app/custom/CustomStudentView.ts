...
import {ClassyStudentView} from "../views/classy/ClassyStudentView";

export default class CustomStudentView extends ClassyStudentView {

    constructor(remoteUrl: string) {
        super(remoteUrl);
        Log.trace("CustomStudentView::<init> - remoteUrl: " + remoteUrl);
    ...
    public async createD3PullRequest() {
        Log.info("SDMMSummaryView::createD3PullRequest() - start");
        UI.showModal('Creating D3 Pull Request<br/>Will take < 10 seconds');

        const url = this.remote + '/portal/sdmm/performAction/patchD3';
        Log.info('SDMMSummaryView::createD3PullRequest( ' + url + ' ) - start');
        ...
        UI.hideModal();
        if (response.status === 200) {
            Log.trace('SDMMSummaryView::createD3PullRequest(..) - 200 received');
            const json = await response.json();

            this.longAction(2000, "D3 Pull Request created. Merge to master when you're ready.");

            this.checkStatus();
        } else {
            Log.trace('SDMMSummaryView::createD3PullRequest(..) - !200 received; status: ' + response.status);
            const json = await response.json();
            this.showError(json);
        }
        return;
